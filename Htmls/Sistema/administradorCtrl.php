...
<head>
	...
	<link rel="shortcut icon" href="../../Recursos/Adm.ico" />
...
<body>
	<div id="agrupar">
		...
		<section id="seccion">
			<form id="form1" name="form1" method="post" action="admistradorCtrl.php">
				<p class="Centro"><h3>AGREGAR EMPLEADO</h3></p>
				<br>
                <img src="../../IMAGENES/User.png" class="img2">
                <?php
                include "administrador.php";
                $adm= new Administrador();
				$adm-> validacionAdmi($_REQUEST['repetir']);
                $adm-> inicializarBD($_REQUEST['nombreusuarios'],$_REQUEST['apepa'],$_REQUEST['apemate'],$_REQUEST['direccion'],$_REQUEST['telefono'],$_REQUEST['edad'],$_REQUEST['email'],$_REQUEST['contrasena'],$_REQUEST['roles']);
                $adm-> conectarBD();
                if($_REQUEST['contrasena']==$_REQUEST['repetir']){
                $adm->registrarAdmi();
                echo '<h1> "USTED ES UN ADMINISTRADOR"</h1>';
                echo'<br>';
                echo '<h2> USTED YA PUEDE REALIZAR CONSULTAS EN SU CAFETERIA"</h2>';
            }
